...
import {
  Clock, AlertTriangle, CheckCircle, Stethoscope, TrendingDown,
  Activity, DollarSign, BarChart2, ChevronDown, ChevronUp,
...
export default function Historial() {
  ...
  useEffect(() => {
    getConsultas()
      ...
      .catch(err => {
        console.error('[Historial] Error al cargar consultas:', err)
  ...
  // Cálculos de presentación: sumas simples sobre datos ya recibidos del backend
  ...
  // ahorroEstimado: suma de (precio_lista - copago). Son datos del backend, solo los sumamos.
    ...
    <div className="historial-container">
      ...
      {consultas.length > 0 && (
        <div className="dashboard-grid">
          ...
          <div className="dashboard-card">
            <div className="dashboard-icon" style={{ background: '#f0fdf4' }}>
          ...
          <div className="dashboard-card">
            <div className="dashboard-icon" style={{ background: '#fefce8' }}>
              <BarChart2 size={22} color="#f59e0b"/>
            </div>
            <div className="dashboard-info">
              <span className="dashboard-label">Especialidad frecuente</span>
              <span className="dashboard-value" style={{ fontSize: '0.95rem' }}>
                {topEspecialidad?.[0] || '—'}
              </span>
            </div>
          </div>
            ...
            return (
              <div key={c.id} className={`consulta-card ${abierto ? 'consulta-card-open' : ''}`}>
                ...
                <div className="consulta-footer">
                  ...
                  {c.copago_estimado != null && (
                    <span className="tag copago-tag">Copago: ${Number(c.copago_estimado).toFixed(2)}</span>
                  )}
                    ...
                    <div className="detalle-grid">
                      {res.aseguradora && (
                        <div className="detalle-item">
                          ...
                          <div>
                            <span className="detalle-label">Aseguradora</span>
                            <span className="detalle-val">{res.aseguradora}</span>
                          </div>
                        </div>
                      )}
                      {res.plan_nombre && (
                        <div className="detalle-item">
                          ...
                          <div>
                            <span className="detalle-label">Plan</span>
                            <span className="detalle-val">{res.plan_nombre}</span>
                          </div>
                        </div>
                      )}
                      {res.cobertura_aplicada != null && (
                        <div className="detalle-item">
                          ...
                          <div>
                            ...
                            <span className="detalle-val">{res.cobertura_aplicada}%</span>
                          </div>
                        </div>
                      )}
                      {res.hospital && (
                        <div className="detalle-item">
                          ...
                          <div>
                            <span className="detalle-label">Hospital</span>
                            <span className="detalle-val">{res.hospital}</span>
                          </div>
                        </div>
                      )}
                      {res.ciudad_hospital && (
                        <div className="detalle-item">
                          ...
                          <div>
                            ...
                            <span className="detalle-val">{res.ciudad_hospital}</span>
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    {res.hospitales_disponibles?.length > 0 && (
                      <div className="detalle-hospitales">
                        ...
                        {res.hospitales_disponibles.map((h, i) => (
                          <div key={h.nombre ?? i} className="detalle-hospital-row">
